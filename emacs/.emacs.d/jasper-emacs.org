...
[ 最后修订与导出时间： *{{{latest-export-date}}}*  ，总字数： *{{{word-count}}}* ]
...
+ Website: <https://xuchangwei.com/emacs/jasper-emacs.html>
+ Git repositories:
...
** 预安装程序
:PROPERTIES:
:CUSTOM_ID: h:97184f00-6c31-4b26-8e03-cc4d4d06738f
:END:
- [[#h:org-spell][aspell]]
- [[#h:c++-treesit-auto][ =init-programming.el= 语法高亮 =treesit-auto= ]] 
- 提前创建 ~/org 目录


* 配置概览
:PROPERTIES:
:CUSTOM_ID: h:277ae533-2af0-44bc-b100-82b7e6564686
:END:

使用 =jasper-emacs.org= 文件来生成配置文件。
- =early-init.el= ，早期初始化文件，放置一些自定义设置，比标准的初始化文件更早生效。
- =init.el= ，它包含我的基础模块系统，即我定义的 Lisp 宏，并加载各个配置模块。
- =lisp= 目录，它包含了我所有的模块配置模块。每个模块都针对特定类型的配置模块。
- =j-lisp= 目录，这里是我自定义的库。
- =jasper-emacs-pre-custom.el= 文件，它在 /before/ 之前被评估模块已加载。
- =jasper-emacs-post-custom.el= 文件， 与上述文件类似，此文件是适用于我的配置用户。它是在我的其他配置之后评估的。
  设置已加载。用户可以在其中包含他们想要的任何代码。文件用于覆盖现有功能或定义新功能。
...
** =early-init.el= 桌面的frame参数设置
:PROPERTIES:
:CUSTOM_ID: h:2d326dd6-d57d-41ed-b27e-72c569c89c12
:END:
...
*** =init.el= 判断emacs版本
:PROPERTIES:
:CUSTOM_ID: h:9ce8e227-dc8a-4aa0-bfa8-41cb1eaa42e8
:END:
#+begin_src emacs-lisp :tangle "init.el"
...
make-temp-file函数会在/tmp下创建并返回临时文件信息，windows是在 =c:/Users/用户名/AppData/Local/Temp= 目录下创建文件。
#+begin_src emacs-lisp :tangle "init.el"
...
强制指定某些软件的源(见[[https://www.gnu.org/software/emacs/manual/html_node/emacs/Package-Installation.html][Package-Installation]])
#+begin_src emacs-lisp :tangle "init.el"
...
** =init.el= 安装软件包的宏 (~j-emacs-install~)
:PROPERTIES:
:CUSTOM_ID: h:c3481607-1d9f-4bfb-9962-d9436ccfdb9c
:END:

这个j-emacs-install宏可以实现我想要的功能，即从软件包归档文件或直接从源代码安装软件包。以下是一些使用示例，完整的代码以注释的形式呈现。简而言之，该宏会根据实际情况选择使用 `install` package-install、package-vc-install`install` 或 `install` 函数package-vc-install-from-checkout

#+begin_example emacs-lisp
(j-emacs-install modus-themes)
;; => (progn
...
(j-emacs-install modus-themes "https://github.com/protesilaos/modus-themes.git")
;; => (unless (package-installed-p 'modus-themes)
...
(j-emacs-install modus-themes :url "https://github.com/protesilaos/modus-themes.git" :branch "main")
;; => (unless (package-installed-p 'modus-themes)
;; =>   (condition-case-unless-debug err
;; =>       (apply #'package-vc-install
...
(j-emacs-install modus-themes "~/Git/Projects/modus-themes")
;; => (unless (package-installed-p 'modus-themes)
;; =>   (condition-case-unless-debug err
...
(j-emacs-install modus-themes "~/Git/Projects/modus-themes-arbitrary-name-of-dir" "modus-themes")
;; => (unless (package-installed-p 'modus-themes)
;; =>   (condition-case-unless-debug err
;; =>       (apply #'package-vc-install
...
** =init.el= 配置软件包的宏 (~j-emacs-configure~)
:PROPERTIES:
:CUSTOM_ID: h:502c41f8-c764-43d9-9c95-624a027681d8
:END:

这j-emacs-configure是一个简单的封装器 condition-case。其理念是为给定的软件包添加任意配置代码，并捕获任何错误，以便 Emacs 即使没有该软件包的代码也能正常加载。
...
*** =ini-package.el= 中的 ~provide~ 调用
:PROPERTIES:
:CUSTOM_ID: h:641255c9-f35f-49cd-89c8-39b01b826703
:END:

最后，我们 ~provide~ 这个模块。这是 ~require~ 的镜像函数 ([[#h:i-modules][ =init.el= 加载各个模块)]]。
...
*** =init-ui.el= 主题 =doom-themes=
:PROPERTIES:
:CUSTOM_ID: h:ui-doom-emacs
:END:

+ 软件包名称（GNU ELPA）： ~doom-theme~
+ 官方手册：<https://github.com/doomemacs/themes>
+ Git 仓库：
...
*** =ini-ui.el= 中的 ~provide~ 调用
:PROPERTIES:
:CUSTOM_ID: h:e5a892d5-40da-491f-a77e-14761106603d
:END:
...
*** =ini-essentials.el= 中的 ~provide~ 调用
:PROPERTIES:
:CUSTOM_ID: h:7c7d05d5-7734-4cbd-a5a0-98e3408bc4cb
:END:
...
Pyim 一个 emacs 中文输入法，支持全拼，双拼，五笔，仓颉和Rime

需要安装 [[https://github.com/tumashu/pyim][pyim]] 中文输入法 和 [[https://github.com/tumashu/pyim-wbdict][pyim-wbdict]] 五笔词库包
#+begin_src emacs-lisp :tangle "lisp/init-basic.el"
;;; 输入法 (pyim)
(use-package pyim
  :ensure t)
...
输入法选择：
- =C-\= : 启用或关闭已选中的输入方法 (~toggle-input-method~) 。
- =C-x RET C-\ 输入法名 RET= : 为当前缓冲区选择新的输入方法  (~set-input-method~)  。或者 =C-u C-\=

pyim五笔使用， =C-x RET C-\ pyim RET= 切换 pyim 输入法。手动指定全拼或五笔：
- =M-x pyim-default-scheme RET wubi= : 使用五笔
- =M-x pyim-wbdict-v86-enable= : 使用 86 版五笔
...
*** =ini-essentials.el= 中的 ~provide~ 调用
:PROPERTIES:
:CUSTOM_ID: h:fbf21fce-6343-48fa-bed0-261db7faab95
:END:
...
*** =ini-modeline.el= 中的 ~provide~ 调用
:PROPERTIES:
:CUSTOM_ID: h:52656a44-8f51-4156-bc71-a249c681341a
:END:
...
everything工具可以快速搜索本地文件。下载地址： <https://www.voidtools.com/zh-cn/downloads/>
...
*** =ini-completion.el= 中的 ~provide~ 调用
:PROPERTIES:
:CUSTOM_ID: h:db406a59-0bfc-4d80-aadf-36b42cf2aedc
:END:
...
*** =ini-evil.el= 中的 ~provide~ 调用
:PROPERTIES:
:CUSTOM_ID: h:31f2ad3f-a001-4354-8867-74c819d7fbcf
:END:
...
*** =init-tools.el= 超文本管理系统 =hyperbole=
:PROPERTIES:
:CUSTOM_ID: h:6494e2de-2931-4163-8482-997a5942d0fb
:END:
...
这里设置不启动 =hyperbole-mode= 模式，需要时再启用。
#+begin_src emacs-lisp :tangle "lisp/init-tools.el"
...
*** =ini-init-tools.el= 中的 ~provide~ 调用
:PROPERTIES:
:CUSTOM_ID: h:7fc74209-7797-4eb7-ac42-31d2aee64c47
:END:
...
*** =ini-init-window.el= 中的 ~provide~ 调用
:PROPERTIES:
:CUSTOM_ID: h:27c27d56-28f5-42ce-a461-7d634cbb817d
:END:
...
*** =init-org.el= 基础Org配置
:PROPERTIES:
:CUSTOM_ID: h:60da1a64-0afb-4959-884a-ae9a17fbce4c
:END:
#+begin_src emacs-lisp :tangle "lisp/init-org.el"
...
*** =ini-init-org.el= 中的 ~provide~ 调用
:PROPERTIES:
:CUSTOM_ID: h:ee448d35-e55e-40e4-9a67-16de42817a5e
:END:
...
*** =init-persp.el= 窗口 =tab-bar(built-in)=
:PROPERTIES:
...
文档：[[https://www.gnu.org/software/emacs/manual/html_node/emacs/Tab-Bars.html][Tab Bars]]
- =C-x t 2= : 新建一个标签 (~tab-new~) 。可通过自定义变量 ~tab-bar-new-tab-choice~ ，控制新标签中默认显示的缓冲区；也可通过自定义变量 ~tab-bar-tab-name-function~ ，设置新标签的默认命名规则。
  - 通过 =M-x tab-bar-new-tab= 生成一个的 =tab-bar= 。 
- =C-x t 0= : 关闭当前选中的标签 (~tab-close~) 。若标签栏中仅有一个标签，该操作默认无效果，除非将变量 ~tab-bar-close-last-tab-choice~ 自定义为非默认值。
- =C-x t 1= : 关闭当前选中框架中，除当前标签外的所有其他标签。
- 命令 ~tab-undo~ 可恢复最后一个被关闭的标签。
- =C-TAB= : 切换至下一个标签  (~tab-next~) 
...
*** =ini-init-persp.el= 中的 ~provide~ 调用
:PROPERTIES:
:CUSTOM_ID: h:d7943323-c3ad-442b-a5be-89c2ad5d707d
:END:
...
原生支持 [[https://github.com/tree-sitter/tree-sitter][tree-sitter]]，编译时指定 =./configure --with-tree-sitter= ， ~(treesit-available-p)~ 返回 =t= 说明配置成功。但这只是完成了核心部分，不同语言的解析器需要单独安装，可以用别人整理好的。
- 方法1-直接下载文件：https://github.com/emacs-tree-sitter/tree-sitter-langs/releases
- 方法2-自动安装包：https://github.com/renzmann/treesit-auto （本节使用）

Emacs 默认从 ~treesit-extra-load-path~ 指定的路径去加载动态链接库，如果这个值是 =nil= ，它默认会从 ~user-emacs-directory~ 下的 =tree-sitter= 目录内加载。
...
*** =ini-init-programming.el= 中的 ~provide~ 调用
:PROPERTIES:
:CUSTOM_ID: h:4d9aa004-c53c-4043-b9ce-f93971e0ed49
:END:
...
*** =ini-init-keybindings.el= 中的 ~provide~ 调用
:PROPERTIES:
:CUSTOM_ID: h:e520f1fa-6f0d-4322-a78e-719816315557
:END:
...
*** =ini-init-funcs.el= 中的 ~provide~ 调用
:PROPERTIES:
:CUSTOM_ID: h:ca631a71-5f7b-4d45-8697-8c6aafb68f04
:END:
