...
highlight-global是github插件，使用quelpa插件辅助安装[[#h:packages-quelpa][(=init-packages.el= 安装 quelpa 插)]] , 自定义了高亮函数[[#h:funcs-symbol-overl][(=init-funcs.el= symbol-overlay & highlight-global)]]做为快捷键引用[[#h:tools-expand-region][(=init-tools.el= expand-region)]]
