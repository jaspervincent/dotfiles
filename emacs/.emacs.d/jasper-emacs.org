...
- ox-hugo来写博客
- eglot emacs29版本内置[[#h:c++-eglot][(使用 eglot 来编写、运行和调试 C/C++ 代码)]]
*** evil
:PROPERTIES:
:CUSTOM_ID: h:learn-evil
:END:
...
*** 精选 packages, 大大提升你的 Emacs 编辑效率
:PROPERTIES:
:CUSTOM_ID: h:learn-module-up-packages
:END:

- 多光标操作 iedit & evil-multiedit[[#h:tools-iedit][(=init-tools.el= 多光标操作 iedit & evil-multiedit)]]
- expand-region[[#h:tools-expand-region][(=init-tools.el= expand-region)]] [[#h:funcs-expand-region][(=init-funcs.el= expand-region)]]
- interactive replace交互式替换文本[[#h:funcs-evil-quick-replace][(=init-funcs.el= interactive replace)]]
- 安装 quelpa 插件[[#h:packages-quelpa][(=init-packages.el= 安装 quelpa 插件)]]
- symbol-overlay & highlight-global[[#h:funcs-symbol-overly][(=init-funcs.el= symbol-overlay & highlight-global)]] [[#h:tools-symbol-overlay][(=init-tools.el= symbol-overlay & highlight-global)]]


*** 使用 Treesit + eglot 来打造现代编程 IDE
:PROPERTIES:
:CUSTOM_ID: h:learn-c++-treesit
:END:
...
适合vim习惯用户, 让minibbuffer上下移动变为 =C-k= =C-j= ,默认minibuffer上下移动是 =C-p= =C-n=
#+begin_src emacs-lisp :tangle "lisp/init-completion.el"
...
添加一个快捷键[[#h:evil-custom-keybinding][(=init.evil.el= 自定义快捷键)]]，让标记和搜索功能更方便[[#h:funcs-expand-region][(=init-funcs.el= expand-region)]]： 
...
- =SPC v e= 等同于evil multiedit中的 R 操作
- =SPC v f= 调用 ~find-file~ 函数查找文件
- =SPC v /= 查找当前项目目录中匹配选中字符串的文件

*** =init-tools.el= symbol-overlay & highlight-global
:PROPERTIES:
:CUSTOM_ID: h:tools-symbol-overlay
:END:
...
highlight-global是github插件，使用quelpa插件辅助安装[[#h:packages-quelpa][(=init-packages.el= 安装 quelpa 插)]] , 自定义了高亮函数[[#h:funcs-symbol-overly][(=init-funcs.el= symbol-overlay & highlight-global)]]做为快捷键引用[[#h:tools-expand-region][(=init-tools.el= expand-region)]]
...
安装完之后运行 =M-x treesit-auto-install-all= , 它自动下载各种语言的 =.dll= 文件, 文件安装在 =~/.emacs.d/tree-sitter=
...
*** =init-funcs.el= expand-region
:PROPERTIES:
:CUSTOM_ID: h:funcs-expand-region
:END:
...
*** =init-funcs.el= interactive replace
:PROPERTIES:
:CUSTOM_ID: h:funcs-evil-quick-replace
:END:
...
*** =init-funcs.el= symbol-overlay & highlight-global
:PROPERTIES:
:CUSTOM_ID: h:funcs-symbol-overly
:END:
